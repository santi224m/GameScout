...
class db_utils:
  ...
  def insert_watchlist_item(username, steamapp_id):
    """Insert a watchlist item into the database"""
    with db_conn() as curr:
      curr.execute("INSERT INTO watchlist_item (user_uuid, steam_app_id) VALUES ((SELECT uuid FROM user_account WHERE username = %s), %s) ON CONFLICT (user_uuid, steam_app_id) DO NOTHING;", (username, steamapp_id))

  def delete_watchlist_item(username, steamapp_id):
    """Remove a watchlist item from a user's account"""
    with db_conn() as curr:
      curr.execute("DELETE FROM watchlist_item WHERE user_uuid = (SELECT uuid FROM user_account WHERE username =%s) AND steam_app_id = %s;", (username, steamapp_id))
  ...
  def get_user_watchlist_items(username):
    """Return a list of steam app ids from a user's wishlist"""
    with db_conn() as curr:
      curr.execute("SELECT steam_app_id, added_date FROM watchlist_item WHERE user_uuid = (SELECT uuid FROM user_account WHERE username = %s);", (username,))
      res = curr.fetchall()
      return res
  ...
  def is_game_watchlisted(username, steamapp_id):
    """Returns True if a user wishlisted the game with steamapp id, False otherwise"""
    with db_conn() as curr:
      curr.execute("SELECT 1 FROM watchlist_item WHERE user_uuid = (SELECT uuid FROM user_account WHERE username = %s) AND steam_app_id = %s;", (username, steamapp_id))
      res = curr.fetchall()
      if res:
        return True
      return False
